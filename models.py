...
class ESTGCN (nn.Module):
    ...
    def forward(self, data):
            ...
            #在rouab优化邻接矩阵的前提下，进行限定阶数的spmm应用
            input = torch.spmm(adj**3, input)
            #第一层卷积操作
            input = self.act(self.gc1(input, edge_index))
...
class MVHGNN(nn.Module):
    def __init__(self, tau, lam, ns_dims, d_ms_dims, t_ms_dims, embedding_dim=128, dropout_rate=0.1):
        ...
        self.feature_graph_attention1 = ESTGCN(d_ms_dims[0], 256, 256, 3)
        self.feature_graph_attention2 = GIN(t_ms_dims[0], 64, 256)
        # self.semantic_graph_attention1 = ESTGCN(d_ms_dims[0], 256, 256, 3)
        # self.semantic_graph_attention2 = GIN(t_ms_dims[0], 64, 256)
        self.drug_graph_attention = CommonFeature(d_ms_dims[0], 64)
        self.target_graph_attention = CommonFeature(t_ms_dims[0], 64)
