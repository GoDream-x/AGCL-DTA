...
pro_res_table = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'Y',
                 'X']
#A-丙氨酸(Alanine) C-半胱氨酸(Cysteine) D-天冬氨酸(Aspartic acid) E-谷氨酸(Glutamic acid) F-苯丙氨酸(Phenylalanine) G-甘氨酸(Glycine)
#H-组氨酸(Histidine) I-异亮氨酸(Isoleucine) K-赖氨酸(Lysine) L-亮氨酸(Leucine) M-甲硫氨酸(Methionine) N-天冬酰胺(Asparagine)
#P-脯氨酸(Proline) Q-谷氨酰胺(Glutamine) R-精氨酸(Arginine) S-丝氨酸(Serine) T-苏氨酸(Threonine) V-缬氨酸(Valine)
#W-色氨酸(Tryptophan)  Y-酪氨酸(Tyrosine) X-未知或任何氨基酸(Unknown or any amino acid)
